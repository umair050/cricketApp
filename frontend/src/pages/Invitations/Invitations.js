...
const Invitations = () => {
  ...
  const getStatusCounts = (invitationList) => {
    return {
      ...
      accepted: invitationList.filter((inv) => inv.status === "ACCEPTED")
        .length,
      rejected: invitationList.filter((inv) => inv.status === "REJECTED")
        .length,
      cancelled: invitationList.filter((inv) => inv.status === "CANCELLED")
        .length,
      expired: invitationList.filter((inv) => inv.status === "EXPIRED").length,
    };
  };

  const receivedCounts = getStatusCounts(received);
  const sentCounts = getStatusCounts(sent);
